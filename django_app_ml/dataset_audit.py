...
from .schema.audit import AuditReport, BasicInfo, MissingValues, DescriptiveStats, CategoricalStats, DescriptiveStatsColumn, CategoricalStatsColumn
...
class PandasDatasetAuditor:
    ...
    def get_basic_info(self, df) -> BasicInfo:
        """
        ...
        try:
            ...
            return BasicInfo(**convert_numpy_types(basic_info))
        except Exception as e:
            logger.error(f"Erreur lors de l'obtention des informations de base: {e}")
            raise

    def get_missing_values(self, df) -> MissingValues:
        """
        ...
        try:
            ...
            return MissingValues(convert_numpy_types(missing_counts))
        except Exception as e:
            logger.error(f"Erreur lors de l'analyse des valeurs manquantes: {e}")
            raise

    def get_descriptive_stats(
        self, df, numeric_columns: Optional[List[str]] = None
    ) -> DescriptiveStats:
        """
        ...
        try:
            ...
            for col in numeric_columns:
                if col in df.columns:
                    ...
                    stats[col] = DescriptiveStatsColumn(
                        mean=col_stats["mean"],
                        std=col_stats["std"],
                        min=col_stats["min"],
                        max=col_stats["max"],
        ...
        try:
            ...
            for col in categorical_columns:
                if col in df.columns:
                    ...
                    stats[col] = CategoricalStatsColumn(
                        unique_count=unique_count,
                        top_values=value_counts
                    )
            return CategoricalStats(stats)
        except Exception as e:
        ...
        try:
            ...
            if save_report:
                ...
                with open(report_path, "w", encoding="utf-8") as f:
                    json.dump(audit_results.model_dump(), f, indent=2, default=convert_numpy_types)
